...
import type { PublicStore } from "@/lib/public-stores";
...
function mapApiBusinessToBusiness(
  business: ApiBusiness | PublicStore,
  index: number,
): Business {
  const legacyName = "nombre" in business ? business.nombre : undefined;
  const legacyCity = "ciudad" in business ? business.ciudad : undefined;
  const category =
    ("category" in business ? business.category : undefined) ??
    business.category_name ??
    (Array.isArray(business.categories) ? business.categories[0] : undefined) ??
    ("giro" in business ? business.giro : undefined) ??
    ("business_category_name" in business
      ? business.business_category_name
      : undefined) ??
    "Restaurante";

  const logoUrl = normalizeBusinessImageCandidate(business.logo_url);
  const imageUrl = normalizeBusinessImageCandidate(business.image_url);
  ...
  const avatarUrl = normalizeBusinessImageCandidate(business.avatar_url);

  return {
    id: business.id,
    name: business.name ?? legacyName ?? "Negocio local",
    city: business.city ?? legacyCity,
    category,
...
export default function ShopPageClient({
  ...
  const [businesses, setBusinesses] = useState<Business[]>(() =>
    initialStores.map((business, index) =>
      mapApiBusinessToBusiness(business, index),
    ),
  );
  const [selectedFilter, setSelectedFilter] = useState("Todos");
  const [searchQuery, setSearchQuery] = useState("");
  const [loading, setLoading] = useState(
    initialStores.length === 0 && !initialStoresError,
  );
  ...
  const [storesWarning, setStoresWarning] = useState(initialStoresError);
  const [_activeOrdersWarning, setActiveOrdersWarning] = useState("");
  const usedServerStateRef = useRef(
    initialStores.length > 0 || Boolean(initialStoresError),
  );
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      if (!query && usedServerStateRef.current) {
        usedServerStateRef.current = false;
        setLoading(false);
        return;
      }
